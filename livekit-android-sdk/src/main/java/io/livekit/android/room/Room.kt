...
constructor(
     ...
     *
     * This value is generated locally when the [Room] is created. Prefer [sid] or [name] when they
     * are available; use [localId] only when a local fallback identifier is needed before server
     * room information has been populated.
     */
     ...
     * that calls `getStats()` on the underlying PeerConnection, which is a
     * synchronous BlockingCall onto the WebRTC signaling/worker threads. During
     ...
     */
    fun disconnect() {
        engine.client.resetQuicHealthForNewSession()
        if (state == State.DISCONNECTED) {
            return
        }
